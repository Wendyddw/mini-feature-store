...
/** In-memory implementation of Fetchers for testing.
  *
  * Reads DataFrames from TestWriter's in-memory storage, making all data operations happen in
  * memory (dictionary) for fast, isolated tests.
  *
  * @param testWriter
  *   TestWriter instance that stores test data
  * @param spark
  *   SparkSession used to convert stored data back to DataFrames
  */
class TestFetcher(testWriter: TestWriter, spark: SparkSession) extends Fetchers {
  ...
  /** Retrieves stored data as a DataFrame.
    *
    * @param key
    *   The storage key (path or table name)
    * @return
    *   Option containing DataFrame if data exists, None otherwise
    */
  def getStoredData(key: String): Option[DataFrame] =
    for {
      rows   <- testWriter.db.get(key)
      schema <- testWriter.dbSchema.get(key)
    } yield spark.createDataFrame(spark.sparkContext.parallelize(rows), schema)
