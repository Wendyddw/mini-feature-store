...
/** Factory object for creating Spark platform instances.
  *
  * Provides convenient methods to create configured SparkPlatform instances with SparkSession and
  * Writers. This follows the Factory Pattern to abstract platform creation logic.
  *
  * Common use patterns:
  * {{{
...
object PlatformProvider {
  ...
  /** Creates a Spark platform with configurable master and settings.
    *
    * @param appName
    *   Name of the Spark application (appears in Spark UI)
    * @param master
    *   Spark master URL (e.g., "yarn", "local[*]", "spark://host:port"). If None, uses default from
    *   spark-submit or spark-defaults.conf
    * @param config
    *   Additional Spark configuration options as key-value pairs
    * @param fetcher
    *   Fetcher implementation to use (defaults to ProdFetcher for production)
    * @param writer
    *   Writer implementation to use (defaults to ProdWriter for production)
    * @return
    *   Configured SparkPlatformTrait instance with SparkSession, Fetcher, and Writer
    *
    * @example
    *   {{{
    *   // Cluster deployment
  ...
  /** Creates a local Spark platform for development and testing.
    *
    * Sets master to "local[*]" to use all available CPU cores locally. This is ideal for local
    * development, unit testing, and debugging.
    *
    * @param appName
    *   Name of the Spark application
    * @param config
    *   Additional Spark configuration (commonly used for testing)
    * @param fetcher
    *   Fetcher implementation (use TestFetcher for in-memory testing)
    * @param writer
    *   Writer implementation (use TestWriter for in-memory testing)
    * @return
    *   Configured SparkPlatformTrait instance for local execution
    *
    * @example
    *   {{{
    *   // Basic local usage
