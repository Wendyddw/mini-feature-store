...
/** Trait defining the interface for data writing operations.
  *
  * Provides abstraction for writing DataFrames to various storage formats and systems.
  * Implementations include ProdWriter (real storage) and TestWriter (in-memory for testing).
  *
  * Common use patterns:
  * {{{
...
trait Writers {
  ...
  /** Writes a DataFrame to Parquet format.
    ...
    * @param partitionBy
    *   Column names to partition by (default: empty, no partitioning)
    */
  def writeParquet(
    df: DataFrame,
    path: String,
  ...
  /** Writes a DataFrame to an Iceberg table using INSERT OVERWRITE semantics.
    *
    * This method performs an atomic overwrite of the table, replacing all existing data. For
    * feature stores, this is commonly used for point-in-time feature snapshots.
    *
    * @param df
    *   DataFrame to write
    * @param tableName
    *   Fully qualified table name (e.g., "db.feature_table")
    * @param partitionBy
    *   Column names to partition the table by (default: empty)
    *
    * @example
    *   {{{
