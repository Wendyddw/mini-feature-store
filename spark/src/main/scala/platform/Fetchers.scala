...
/** Trait defining the interface for data reading operations.
  *
  * Provides abstraction for reading DataFrames from various storage formats and systems.
  * Implementations include ProdFetcher (real storage) and TestFetcher (in-memory for testing).
  */
trait Fetchers {
  ...
  /** Reads a DataFrame from Parquet format.
    *
    * @param spark
    *   SparkSession for reading
    * @param path
    *   Path to Parquet files
    * @param schema
    *   Optional schema to apply (default: None, infer from data)
    */
  def readParquet(
    spark: SparkSession,
    path: String,
