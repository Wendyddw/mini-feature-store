...
/** Configuration for PointInTimeJoinPipeline.
  *
  * @param labelsPath Path to labels table/data (Parquet format)
  * @param featuresTable Iceberg table name for features_daily
  * @param outputPath Output path for joined training data (Parquet format)
  * @param partitionBy Columns to partition the output by (default: Seq("as_of_ts"))
  *
  * @example
  * {{{
